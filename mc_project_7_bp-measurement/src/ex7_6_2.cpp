...
void loop() {
    ...
    for (int i = 0; i < circSeg; i++) {
        fillArc(arcPosy, arcPosx, arcRady + arcThick, arcRadx + arcThick, circSeg, i,
            white);
    }
    for (int i = 0; i < circSeg; i++) {
        fillArc(arcPosy, arcPosx, arcRady, arcRadx, circSeg, i, black);
    }
    display.fillRect(arcPosx - (arcRadx + arcThick), arcPosy, 2 * (arcRadx +
        arcThick), arcThick, white);
    //Fill in the bars for the beart beat strength
    for (int i = 0; i < 8; i++) {
        display.fillRect(barWidth * i + barWidth / 2, barPosy, barWidth - 1,
            barHeight, black);
    }
    ...
    for (int i = 0; i < measPointMax; i++) {
        ...
        if (currentPressure < maxPressure && currentPressure > minPressure) {
            ...
            intermediateSegment[0] = circSeg - round(circSlope * currentPressure -
                circOffset);
            if (intermediateSegment[0] >= intermediateSegment[1]) {
                indicator(arcPosy, arcPosx, arcRady, arcRadx, circSeg,
                    intermediateSegment[0], 5, green);
            } else {
                indicator(arcPosy, arcPosx, arcRady, arcRadx, circSeg,
                    intermediateSegment[1], 5, black);
            }
            //Heart beat strength with threshold
            if (HPPressure > barThreshold) {
                heartStrength = HPPressure - barThreshold;
                drawTime = i;
                for (int k = drawBeat; k < 9; k++) {
                    if (k * 100 <= heartStrength) {
                        display.fillRect(barWidth * k + barWidth / 2, barPosy, barWidth - 1,
                            barHeight, colorGradient[8 - k]);
                        drawBeat = k;
                    }
                }
            } else if ((i - drawTime) % 10 == 0) { //remove blocks every 100ms
                display.fillRect(barWidth * drawBeat + barWidth / 2, barPosy, barWidth -
                    1, barHeight, black);
                if (drawBeat > 0) {
                    drawBeat -= 1;
                }
            ...
            if (i % 50 == 0) {
                drawNumber(printPressure1, xCursor, yCursor, 3, green, black); //4*7
                grid times 3
...
void indicator(int x, int y, int rx, int ry, int seg_count, int seg, int size,
    unsigned int colour) {
    float sx1 = cos(pi * seg / seg_count + pi / 2);
    float sy1 = sin(pi * seg / seg_count + pi / 2);
    uint16_t x1 = sx1 * rx + x;
    ...
    for (int l = 0; l <= squareNumber; l++) {
        display.fillRect(sy1 * ry * l / squareNumber + y - size / 2 - 1, sx1 * rx * l /
            squareNumber + x - size / 2 - 1, size + 2, size + 2, colour);
    }
}
