...
void loop() {
  ...
  if (choosePressure) {
    ...
    if (myFile) {
      ...
      while (myFile.available() && digitalRead(readPressureButton) == LOW) {  // read from the file until there's nothing else in it
    ...
    if (myFile) {
      ...
      while (myFile.available() && digitalRead(readHPPressureButton) == LOW) {  // read from the file until there's nothing else in it
